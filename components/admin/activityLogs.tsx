...
export default function LogTable() {
  ...
  const parseAction = (context: string): string => {
    if (context.toLowerCase().includes('reservation')) {
      return 'Reservation'
    } else if (context.toLowerCase().includes('book')) {
      return 'Book Update'
    } else if (context.toLowerCase().includes('provider')) {
  ...
  return (
    <div className="w-4/5 mx-auto p-6 space-y-6 bg-background rounded-lg shadow-md">
      <div className="flex flex-col md:flex-row justify-between items-center gap-4">
        <h2 className="text-2xl font-bold text-primary">Logs List</h2>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <Button
          onClick={() => handleFilterByAction('Book Update')}
          variant="outline"
        ...
        <Button
          onClick={() => handleFilterByAction('Provider Update')}
          variant="outline"
                    ...
                    <span
                      className={`px-2 py-1 rounded-full text-xs font-medium ${
                        log.Action === 'Reservation'
                          ...
                          : log.Action === 'Book Update'
                          ? 'bg-green-100 text-green-700'
                          : log.Action === 'Provider Update'
